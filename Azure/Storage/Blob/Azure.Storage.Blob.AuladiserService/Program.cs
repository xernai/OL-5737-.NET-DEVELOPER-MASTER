...
namespace Azure.Storage.Blob.AuladiserService
{
    ...
    public class Program
    {
        static async Task Main(string[] args)
        {
            ...
            var result = await azureBlobStorageService.ListBlobsAsync("ecommerce");

            if (result == null || result.Count == 0)
            {
                Console.WriteLine("No blobs found in container 'ecommerce'.");
            }
            else
            {
                Console.WriteLine($"Found {result.Count} blob(s) in container 'ecommerce':");
